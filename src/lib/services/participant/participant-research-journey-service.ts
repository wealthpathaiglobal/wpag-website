...
export class ParticipantResearchJourneyService {
  ...
  async decide(input: { participantId: string; actorUserId: string; decision: "GRANTED" | "DECLINED"; directConsent: boolean; baselineConsent: boolean; followUpScopeDecision: FollowUpScopeDecisionInput; acknowledgements: Record<string, boolean>; presentationEventId: string; presentedArtifactVersion: string; presentedArtifactSha256: string; presentedAt: string; correlationId: string }) {
    ...
    try {
      ...
      if (journey.consentPresentationEventId !== input.presentationEventId || journey.consentArtifactVersion !== input.presentedArtifactVersion || journey.consentArtifactSha256 !== input.presentedArtifactSha256 || journey.consentPresentedAt !== input.presentedAt) throw new ParticipantResearchJourneyServiceError("invalid");
      return await this.repository.decide({ ...input, enrollmentId: journey.enrollmentId });
    } catch (error) { if (error instanceof ParticipantResearchJourneyServiceError) throw error; safe(error); }
  }
}
