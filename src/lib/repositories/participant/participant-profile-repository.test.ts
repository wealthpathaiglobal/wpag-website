...
describe("participant profile repository", () => {
  ...
  it("saves with exact governed parameters and no participant identity",async()=>{await saveCurrentParticipantProfile(input);expect(mocks.rpc).toHaveBeenCalledWith("save_current_participant_profile",expect.objectContaining({p_first_name:"Asha",p_country_code:"IN"}));expect(JSON.stringify(mocks.rpc.mock.calls[0])).not.toContain("participant_id");});
  it("completes with the no-argument governed command",async()=>{await completeCurrentParticipantProfile();expect(mocks.rpc).toHaveBeenCalledWith("complete_current_participant_profile");});
