...
const prepareRow = { document_id: documentId, assessment_id: assessmentId, storage_bucket: "assessment-evidence", storage_path: path, original_filename: "statement.pdf", mime_type: "application/pdf", file_size_bytes: 10, sha256, version_number: 1 };
...
describe("ParticipantEvidenceFoundationRepository", () => {
  ...
  it("maps the exact prepare RPC contract", async () => {
    ...
    await expect(new ParticipantEvidenceFoundationRepository().prepare(input)).resolves.toMatchObject({ documentId, storageBucket: "assessment-evidence", sha256 });
  ...
  it("uploads only to the private bucket without overwrite", async () => {
    ...
    await repository.upload({ documentId, assessmentId, storageBucket: "assessment-evidence", storagePath: path, originalFilename: "statement.pdf", mimeType: "application/pdf", fileSizeBytes: 10, sha256, versionNumber: 1 }, new Uint8Array(10));
  ...
  it("finalizes through the governed RPC and never writes a table directly", async () => {
    ...
    const reservation = { documentId, assessmentId, storageBucket: "assessment-evidence" as const, storagePath: path, originalFilename: "statement.pdf", mimeType: "application/pdf" as const, fileSizeBytes: 10, sha256, versionNumber: 1 };
    await expect(new ParticipantEvidenceFoundationRepository().finalize(input, reservation)).resolves.toMatchObject({ documentId, verificationStatus: "pending" });
    expect(supabaseAdminSpies.rpc).toHaveBeenCalledWith("finalize_evidence_upload", expect.objectContaining({ p_document_id: documentId, p_storage_path: path, p_sha256: sha256 }));
