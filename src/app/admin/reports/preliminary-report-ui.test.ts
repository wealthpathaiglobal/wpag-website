...
describe("preliminary report UI contracts", () => {
  ...
  it("updates the dashboard only from durable report availability", () => { const loader = source("src/lib/services/participant/participant-dashboard-loader.ts"); const policy = source("src/app/participant/dashboard/participant-dashboard-policy.ts"); expect(loader).toContain("listParticipantPreliminaryReports"); expect(policy).toContain('input.reportAvailable ? "Released report available" : "No released report available"'); expect(policy).toContain('href: "/participant/reports"'); });
