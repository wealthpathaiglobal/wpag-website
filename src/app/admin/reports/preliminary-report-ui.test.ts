...
describe("preliminary report UI contracts", () => {
  ...
  it("updates the dashboard only from durable report availability", () => { const page = source("src/app/participant/dashboard/page.tsx"); const policy = source("src/app/participant/dashboard/participant-dashboard-policy.ts"); expect(page).toContain("listParticipantPreliminaryReports"); expect(policy).toContain('input.reportAvailable ? "Released report available" : "No released report available"'); expect(policy).toContain('href: "/participant/reports"'); });
