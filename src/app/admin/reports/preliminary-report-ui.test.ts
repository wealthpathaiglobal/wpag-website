...
describe("preliminary report UI contracts", () => {
  ...
  it("shows preliminary non-advice notices above and below report content", () => { const detail = source("src/app/participant/reports/[reportId]/page.tsx"); expect((detail.match(/<aside/g) ?? []).length).toBe(2); expect(detail).toContain("preliminary research report"); expect(detail).toContain("not financial advice"); expect(detail).toContain("No PDF download"); });
