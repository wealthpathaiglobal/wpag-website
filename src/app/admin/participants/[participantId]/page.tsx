...
export async function GovernedProjection<T>({ promise, boundary = "governed-projection", label, children }: { promise: Promise<T>; boundary?: string; label: string; children: (value: T) => ReactNode }) {
...
export default async function ParticipantDetailPage({ params }: ParticipantDetailPageProps) {
  ...
  recordAdminServerPerformance("participant-workspace", "request_start", timing.startedAt);
  const staff = await requireAdminAccess("/admin/dashboard");
  recordAdminServerPerformance("participant-workspace", "administrator_authorization_complete", timing.startedAt);
  const { participantId } = await params;
  const participant = await getParticipantCore(participantId);
  recordAdminServerPerformance("participant-workspace", "participant_core_shell_ready", timing.startedAt);
  const loads = startParticipantProjectionLoads(participantId, staff.auth_user_id);
  recordAdminServerPerformance("participant-workspace", "server_stream_render_point", timing.startedAt);

  return <main className="min-h-screen bg-black px-4 py-10 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl">
    <AdminPerformanceTelemetry route="participant-workspace" startedAtEpochMs={timing.startedAtEpochMs} expectedBoundaryCount={9} />
    ...
    <Suspense fallback={<ProjectionFallback label="Assessment summary" />}><GovernedProjection promise={loads.assessmentSummary} boundary="assessment-summary" label="Assessment summary">{(summary) => <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6" aria-label="Assessment summary"><p className="text-xs uppercase tracking-wider text-white/35">Participant assessment</p><div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-white/40">Status</p><p className="mt-2">{formatStatus(summary?.session_status ?? null)}</p></div><div><p className="text-xs text-white/40">Modules</p><p className="mt-2">{summary ? `${summary.completed_module_count} of ${summary.total_module_count}` : "—"}</p></div><div><p className="text-xs text-white/40">Versions</p><p className="mt-2">{summary ? `${summary.assessment_version} · ${summary.hfos_version}` : "—"}</p></div><div><p className="text-xs text-white/40">Submitted</p><p className="mt-2">{formatDate(summary?.submitted_at ?? null)}</p></div></div>{summary?.session_status === "submitted" && summary.assessment_id ? <Link prefetch={false} href={`/admin/reviews/assessments/${summary.assessment_id}`} className="mt-6 inline-flex rounded-xl bg-sky-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-sky-500">Open Assessment Review</Link> : null}</section>}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Measurement summary" />}><GovernedProjection promise={loads.measurementSummary} boundary="measurement-summary" label="Measurement summary">{(summary) => <HfosMeasurementFoundationCard summary={summary} />}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Research controls" />}><GovernedProjection promise={loads.researchControls} boundary="research-controls" label="Research controls">{(status) => <ResearchControlsFoundationCard status={status} />}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Research requests" />}><GovernedProjection promise={loads.researchRequests} boundary="research-requests" label="Research requests">{(requests) => <ResearchParticipantRequestPanel participantId={participantId} requests={requests} />}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Wave 3 governance" />}><GovernedProjection promise={loads.researchWave3} boundary="wave-3" label="Wave 3 governance">{(overview) => <ResearchWave3GovernanceCard overview={overview} />}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Wave 4 readiness" />}><GovernedProjection promise={loads.researchWave4} boundary="wave-4" label="Wave 4 readiness">{(overview) => <ResearchWave4ReadinessCard participantId={participantId} overview={overview} />}</GovernedProjection></Suspense>
    <Suspense fallback={<ProjectionFallback label="Evidence" />}><GovernedProjection promise={loads.evidence} boundary="evidence" label="Evidence">{(evidence) => <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6" aria-label="Participant evidence"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Evidence</h2><p className="mt-1 text-sm text-white/45">Governed assessment evidence and current verification status.</p></div><Link prefetch={false} href="/admin/evidence" className="text-sm text-sky-300">Open Evidence Queue</Link></div>{evidence.length === 0 ? <p className="mt-5 text-sm text-white/45">No evidence is available for this participant.</p> : <div className="mt-5 space-y-3">{evidence.map((item) => <Link prefetch={false} key={item.documentId} href={`/admin/evidence/${item.documentId}`} className="flex flex-col gap-2 rounded-xl border border-white/10 p-4 hover:bg-white/[0.04] sm:flex-row sm:items-center sm:justify-between"><span>{item.documentName} · v{item.versionNumber}</span><span className="text-sm text-white/50">{formatStatus(item.verificationStatus)}</span></Link>)}</div>}</section>}</GovernedProjection></Suspense>

    <Suspense fallback={<ProjectionFallback label="Invitation controls" />}><GovernedProjection promise={loads.invitation} boundary="invitation" label="Invitation controls">{(invitation) => <ParticipantInvitationPanel participantId={participant.id} authUserId={participant.auth_user_id} lifecycleStatus={participant.lifecycle_status} invitation={invitation} />}</GovernedProjection></Suspense>
    <LifecycleActionPanel participantId={participant.id} lifecycleStatus={participant.lifecycle_status} />
    <Suspense fallback={<ProjectionFallback label="Lifecycle timeline" />}><GovernedProjection promise={loads.lifecycleHistory} boundary="lifecycle-timeline" label="Lifecycle timeline">{(history) => <section className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]"><div className="border-b border-white/10 px-6 py-5"><h2 className="text-lg font-semibold">Lifecycle Timeline</h2><p className="mt-1 text-sm text-white/45">Immutable history of participant lifecycle transitions.</p></div>{history.length === 0 ? <div className="px-6 py-14 text-center"><p className="text-sm text-white/50">No lifecycle history is available.</p></div> : <div className="divide-y divide-white/5">{history.map((event) => <article key={event.id} className="grid gap-4 px-6 py-5 md:grid-cols-[180px_1fr]"><time dateTime={event.changed_at} className="text-sm text-white/45">{formatDate(event.changed_at)}</time><div><div className="flex flex-wrap items-center gap-2">{event.from_status ? <><span className="text-sm text-white/45">{formatStatus(event.from_status)}</span><span className="text-white/25" aria-hidden="true">→</span></> : null}<span className="font-medium text-white">{formatStatus(event.to_status)}</span></div>{event.transition_reason ? <p className="mt-2 text-sm leading-6 text-white/55">{event.transition_reason}</p> : null}{event.changed_by ? <p className="mt-2 text-xs text-white/30">Changed by: {event.changed_by}</p> : null}</div></article>)}</div>}</section>}</GovernedProjection></Suspense>
  </div></main>;
}
