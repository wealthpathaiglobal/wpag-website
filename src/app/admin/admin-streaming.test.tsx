...
describe("admin streamed rendering policy", () => {
  ...
  it("gates projection actions until success and makes failure visible", async () => {
    ...
    const element = await GovernedProjection({ promise: Promise.reject(new Error("unavailable")), label: "Research controls", children: renderAction }) as ReactElement<{ role: string }>;
  ...
  it("renders governed content only after its projection succeeds", async () => {
    ...
    await expect(GovernedProjection({ promise: Promise.resolve("authorized action"), label: "Research controls", children: renderAction })).resolves.toBe("authorized action");
