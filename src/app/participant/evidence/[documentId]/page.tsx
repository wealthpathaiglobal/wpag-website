...
export function participantEvidenceDownloadPolicy(canDownload: boolean) {
  const visible = canDownload === true;
  return { current: visible, historical: visible };
}
...
export default async function ParticipantEvidenceDetailPage({ params }: { params: Promise<{ documentId: string }> }) {
  ...
  const downloadPolicy = participantEvidenceDownloadPolicy(evidence.canDownload);
  return <main className="min-h-screen bg-[#f4f2ed] px-5 py-10 text-black sm:px-8"><div className="mx-auto max-w-5xl">
    ...
    <section className="grid gap-8 py-10 lg:grid-cols-[1.2fr_0.8fr]"><div className="space-y-8"><article className="border border-black/20 bg-white/40 p-6 sm:p-8"><h2 className="font-serif text-3xl">Current version</h2><dl className="mt-6 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-black/50">Filename</dt><dd className="mt-1 break-all">{evidence.originalFilename}</dd></div><div><dt className="text-black/50">Version</dt><dd className="mt-1">{evidence.currentVersion}</dd></div><div><dt className="text-black/50">Submitted</dt><dd className="mt-1">{date(evidence.submittedAt)}</dd></div><div><dt className="text-black/50">Size</dt><dd className="mt-1">{Math.ceil(evidence.fileSizeBytes / 1024)} KiB</dd></div></dl>{evidence.verificationNotes && <div className="mt-6 border-l-2 border-black pl-4"><p className="text-xs uppercase tracking-wider text-black/50">Verification feedback</p><p className="mt-2 text-sm leading-7">{evidence.verificationNotes}</p></div>}{downloadPolicy.current && <a href={`/api/participant/evidence/${evidence.documentId}/download`} className="mt-6 inline-flex border border-black bg-black px-5 py-3 text-xs font-semibold uppercase tracking-wider text-white">Download Current Version</a>}</article>
      <article className="border border-black/20 bg-white/40 p-6 sm:p-8"><h2 className="font-serif text-3xl">Version history</h2><div className="mt-5 divide-y divide-black/10">{evidence.versions.map((version) => <div key={version.versionNumber} className="grid gap-3 py-5 sm:grid-cols-[1fr_auto]"><div><p className="font-medium">Version {version.versionNumber} · {version.originalFilename}</p><p className="mt-1 text-sm text-black/55">{version.mimeType} · {Math.ceil(version.fileSizeBytes / 1024)} KiB · {date(version.submittedAt)}</p></div>{downloadPolicy.historical && <a href={`/api/participant/evidence/${evidence.documentId}/download?version=${version.versionNumber}`} className="text-sm underline">Download</a>}</div>)}</div></article>
